...
### MAIN
def ID3(df, df_training, df_validation, pruning):
    rules = []

    data_training = Data(df_training)

    change_continuous_atributes(df_training, data_training)

    df = pd.DataFrame(data = data_training.data_values)
    df.columns = ["sepal.length","sepal.width","petal.length","petal.width","variety"]

    #print("--------------ID3------------")

    #hasil_id3 = id3(data_training, df)
    #print_tree(hasil_id3,0)

    #print("--------------C45------------")

    hasil_c45 = c45(data_training, df)
    #print_tree(hasil_c45,0)
    print('Tree built using C4.5.')

    if (pruning):
        #print("--------------Hasil Prune------------")
        data_validation = Data(df_validation)
        hasil_diprune = prune(hasil_c45, data_validation, df_validation, rules, False)
        #print_tree(hasil_diprune, 0)
        print('Tree pruned.')
    else:
        hasil_diprune = hasil_c45

    return hasil_diprune